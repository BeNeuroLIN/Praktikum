...
def processAngles(experiment, data, num_bins):
    ...
    tmp = np.nansum(data[group_3], axis=3)
    freq_tmp_3 = np.nanmean(tmp, axis=1)
    freq_data_3 = np.nanmean(freq_tmp_3, axis=0)
    freq_sem_3 = sem(freq_tmp_3, axis=0, nan_policy='omit')
    freq_std_3 = np.nanstd(freq_tmp_3, axis=0)
    ...
    to_save['mean_3'] = avg_data_3
    to_save['sem_3'] = sem_data_3.data if np.ma.isMaskedArray(sem_data_3) else sem_data_3
    to_save['prob_3'] = prob_data_3
    to_save['prob_sem_3'] = prob_sem_3.data if np.ma.isMaskedArray(prob_sem_3) else prob_sem_3
    #added for SEM
    ...
    to_save['freq_3_raw'] = freq_tmp_3
    to_save['freq_3'] = freq_data_3
    to_save['freq_sem_3'] = freq_sem_3.data if np.ma.isMaskedArray(freq_sem_3) else freq_sem_3
    to_save['freq_std_3'] = freq_std_3
...
def boutFrequency(experiment, num_bins):
    ...
    sem_3 = tmp['freq_sem_3']
    raw_1 = tmp['freq_1_raw']
    raw_2 = tmp['freq_2_raw']
    raw_3 = tmp['freq_3_raw']
    ...
    if stimuli % 2 == 0:
        ...
        raw_3 = (raw_3[:, :half] + raw_3[:, half:]) / 2.0
        '''
        ...
        raw_3 = (raw_3[:, :half] + raw_3[:, half:-1]) / 2.0
        '''
    ...
    freq_3 = np.nanmean(raw_3, axis=0)
    sem_3 = sem(raw_3, axis=0, nan_policy='omit')
    ...
    for i in x_range:
        x_1 = np.full(raw_1.shape[0], i - offset)
        x_2 = np.full(raw_2.shape[0], i)
        x_3 = np.full(raw_3.shape[0], i + offset)
...
if __name__ == '__main__':
    ...
    experiment = '2026_04_14_paula_circadianshift'
    num_bins = 72
